...
public class OAUTH extends Activity {
	...
	public void onCreate(Bundle icicle) {
		...
		if (i.getData() == null) {
			try {
				...
				saveRequestInformation(mSettings, mConsumer.getToken(),
	...
	protected void onResume() {
		...
		if (uri != null && CALLBACK_URI.getScheme().equals(uri.getScheme())) {
			...
			Intent i = new Intent(this, Main.class); // Currently, how we get
														// back to main
														// activity.
			...
			try {
				...
				i.putExtra(Constants.USER_TOKEN, token);
				i.putExtra(Constants.USER_SECRET, secret);
			} catch (OAuthMessageSignerException e) {
				e.printStackTrace();
			} catch (OAuthNotAuthorizedException e) {
				e.printStackTrace();
